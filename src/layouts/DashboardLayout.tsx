...
function DashboardHeader() {
  ...
  return (
    <header className="dashboard-app-header flex-shrink-0 fixed top-0 left-0 right-0 z-50 flex items-center px-4 sm:px-6 bg-white border-b border-slate-200/60">
      <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
        <button
          type="button"
          className="dashboard-header-burger md:hidden flex items-center justify-center w-10 h-10 min-w-10 min-h-10 rounded-lg border-0 bg-transparent text-slate-600 hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:ring-inset"
          onClick={toggle}
        ...
        <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200/80">
          {roleLabel}
        </span>
      </div>
