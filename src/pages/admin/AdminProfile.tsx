...
export default function AdminProfile() {
  ...
  const handleSave = () => {
    addEntry({ action: 'profile_updated', actor: 'Admin', target: name, details: 'Profile information updated' })
    setSaved(true)
  ...
  const handleChangePw = (e: React.FormEvent) => {
    ...
    addEntry({ action: 'password_changed', actor: 'Admin', target: name, details: 'Password updated' })
    setPwSaved(true)
    setTimeout(() => {
      setPwSaved(false)
  ...
  const handleToggle2FA = () => {
    ...
    addEntry({ action: next ? '2fa_enabled' : '2fa_disabled', actor: 'Admin', target: name, details: next ? 'Two-factor authentication enabled' : 'Two-factor authentication disabled' })
  }

  const handleRevokeSession = (idx: number) => {
    const s = sessions[idx]
    if (s) {
      addEntry({ action: 'session_revoked', actor: 'Admin', target: s.device, details: s.location })
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        // We need to fetch the employee record to get the full details (fname, lname, role, etc.)
        // because updateEmployee requires the full payload.
        const emps = await fetchEmployees(roleOptions)
        ...
        if (me) {
          ...
          if (result) {
            ...
            addEntry({ action: 'profile_photo_updated', actor: 'Admin', target: name, details: 'Profile photo updated' })
          }
        }
      } catch (err) {
    ...
    <div className="dashboard-page dashboard-page--profile">
        ...
        <div className="profile-grid-left">
          ...
          <section className="dashboard-card">
            ...
            <div className="profile-form">
              <div className="profile-field">
                <label htmlFor="profile-name" className="modal-label">Full name</label>
                <input id="profile-name" type="text" className="modal-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" />
              </div>
              <div className="profile-field">
                <label htmlFor="profile-email" className="modal-label">Email</label>
                <input id="profile-email" type="email" className="modal-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" />
              </div>
              <div className="profile-field">
              ...
              <div className="profile-actions">
                <button
                  type="button"
                  className={`employees-btn employees-btn-primary profile-save-btn ${saved ? 'profile-save-btn--saved' : ''}`}
                  onClick={handleSave}
                >
          ...
          <section className="dashboard-card">
            ...
            <div className="profile-notif-list">
              ...
              <label className="profile-notif-item">
                <div className="profile-notif-text">
                  <span className="profile-notif-label">Approval requests</span>
                  <span className="profile-notif-desc">Get notified about new sign-up requests</span>
                </div>
                <div className={`profile-toggle ${notifApprovals ? 'profile-toggle--on' : ''}`} onClick={() => setNotifApprovals((v) => !v)} role="switch" aria-checked={notifApprovals} tabIndex={0}>
                  <div className="profile-toggle-knob" />
                </div>
              </label>
              <label className="profile-notif-item">
      ...
      {changePwOpen && (
        <div className="modal-backdrop" onClick={() => !pwSaved && setChangePwOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} role="dialog" aria-labelledby="pw-title" aria-modal="true">
            <h2 id="pw-title" className="modal-title">Change password</h2>
            <form className="modal-body" onSubmit={handleChangePw}>
              <div className="modal-field">
                <label htmlFor="pw-current" className="modal-label">Current password</label>
                <div className="profile-pw-wrap">
                  <input id="pw-current" type={showCurrentPw ? 'text' : 'password'} className="modal-input" value={currentPw} onChange={(e) => setCurrentPw(e.target.value)} required autoFocus />
                  <button type="button" className="profile-pw-toggle" onClick={() => setShowCurrentPw((v) => !v)} aria-label={showCurrentPw ? 'Hide' : 'Show'}>
                    {showCurrentPw ? 'Hide' : 'Show'}
                  </button>
                </div>
              </div>
              <div className="modal-field">
                <label htmlFor="pw-new" className="modal-label">New password</label>
                <div className="profile-pw-wrap">
                  <input id="pw-new" type={showNewPw ? 'text' : 'password'} className={`modal-input ${pwTooShort ? 'modal-input--error' : ''}`} value={newPw} onChange={(e) => setNewPw(e.target.value)} required minLength={8} />
                  <button type="button" className="profile-pw-toggle" onClick={() => setShowNewPw((v) => !v)} aria-label={showNewPw ? 'Hide' : 'Show'}>
                    {showNewPw ? 'Hide' : 'Show'}
                  </button>
                </div>
              ...
              <div className="modal-field">
                <label htmlFor="pw-confirm" className="modal-label">Confirm new password</label>
                <div className="profile-pw-wrap">
                  <input id="pw-confirm" type={showConfirmPw ? 'text' : 'password'} className={`modal-input ${pwMismatch ? 'modal-input--error' : ''}`} value={confirmPw} onChange={(e) => setConfirmPw(e.target.value)} required />
                  <button type="button" className="profile-pw-toggle" onClick={() => setShowConfirmPw((v) => !v)} aria-label={showConfirmPw ? 'Hide' : 'Show'}>
